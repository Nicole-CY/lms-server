...
const updateCourseOfferingByIdAsync = async (req, res) => {
    try {
        ...
        if (req.body.teacher_id !== undefined)
            updateData.teacherId = req.body.teacher_id;

        if (req.body.start_date !== undefined) {
            const startDate = new Date(req.body.start_date);
            if (isNaN(startDate.getTime()))
                return res.sendCommonValue({}, "Invalid start_date", 0);
        ...
        if (req.body.end_date !== undefined) {
            ...
            if (isNaN(endDate.getTime()))
                return res.sendCommonValue({}, "Invalid end_date", 0);
        ...
        if (req.body.updated_by !== undefined)
            updateData.updatedBy = req.body.updated_by;

        const result = await CourseOfferingService.updateCourseOfferingByIdAsync(id, updateData);

        if (result.isSuccess) {
            res.sendCommonValue(result.data, result.message || "Course offering updated successfully", 1);
        } else {
