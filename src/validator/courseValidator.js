...
const addCourseValidator = [
    ...
    body('courseCode')
        ...
        .isString()
        .withMessage('courseCode must be a string')
        .isLength({ min: 3, max: 20 })
        .withMessage('courseCode length must be between 3 and 20 characters'),
...
const getCourseByTitleValidator = [query('title').notEmpty().withMessage('title is required')];
...
const deleteCourseValidator = [
    param('id')
        ...
        .isInt({ min: 1 })
        .withMessage('not a valid course id'),
];
