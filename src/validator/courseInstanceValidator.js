...
const getCourseInstanceListValidator = [
    ...
    query('launchStatus')
        ...
        .isIn(['Scheduled', 'In Progress', 'Completed', 'Cancelled'])
        .withMessage('Invalid launch status'),
    query('startDateFrom').optional().isDate().withMessage('Start date from must be a valid date'),
    query('startDateTo').optional().isDate().withMessage('Start date to must be a valid date'),
    query('endDateFrom').optional().isDate().withMessage('End date from must be a valid date'),
    query('endDateTo').optional().isDate().withMessage('End date to must be a valid date'),
];
...
const bulkDeleteCourseInstancesValidator = [
    body('ids')
        ...
        .custom(value => {
            if (
                !value.every(
                    id => Number.isInteger(id) || (typeof id === 'string' && /^\d+$/.test(id))
                )
            ) {
