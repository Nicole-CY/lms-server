...
const addCourseValidator = [
    ...
    body("courseCode").notEmpty().withMessage("courseCode is required").isString().withMessage("courseCode must be a string"),
...
const updateCourseValidator = [
    body("id").notEmpty().withMessage("id is required").isInt({min: 1}).withMessage("id must be a valid integer"),
    body("title").notEmpty().withMessage("title is required").isString().withMessage("title must be a string"),
    body("courseCode").notEmpty().withMessage("courseCode is required").isString().withMessage("courseCode must be a string"),
    body("coverImage").optional().isString().withMessage("Cover image must be a string"),
    body("description").optional().isString().withMessage("Description must be a string"),
...
const getCourseByIdValidator = [
    query("id").notEmpty().withMessage("id is required").isInt({ min: 1 }).withMessage("id must be a valid integer"),
];
const getCourseListValidator = [
    query("page").optional().isInt({allow_leading_zeroes: false, min: 1}).withMessage("Page must be an integer greater than 0"),
    query("pageSize").optional().isInt({allow_leading_zeroes: false, min: 1}).withMessage("Page size must be an integer greater than 0"),
    ...
    query("categories.*").optional().isInt({min: 1}).withMessage("each category id must be a valid integer"),
];

const deleteCourseValidator = [
    param("id").notEmpty().withMessage("course id is required").isInt({ min: 1 }).withMessage("not a valid course id"),
];

const bulkDeleteCoursesValidator = [
    body("ids").notEmpty().withMessage("ids are required").isArray({min: 1}).withMessage("ids must be a non-empty array"),
