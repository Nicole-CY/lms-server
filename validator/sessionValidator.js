...
const addSessionValidator = [
    body("courseInstanceId")
        ...
        .isInt({ min: 1 }).withMessage("Course Instance ID must be an integer"),
    body("sessionTitle")
        .notEmpty().withMessage("session title is required")
        .isString().withMessage("session title must be a string")
        .isLength({ min: 3, max: 50 }).withMessage("session title length must be between 3 and 50 characters"),
...
const updateSessionValidator = [
    ...
    body("courseInstanceId")
        .notEmpty().withMessage("Course Instance ID is required")
        .isInt().withMessage("Course Instance ID must be an integer"),
    body("sessionTitle")
        .notEmpty().withMessage("session title is required")
        .isString().withMessage("session title must be a string")
        .isLength({ min: 3, max: 50 }).withMessage("session title length must be between 3 and 50 characters"),
...
const getSessionsByCourseInstanceIdValidator = [
    query("courseInstanceId").notEmpty().withMessage("course instanceid is required").isInt({ min: 1 }).withMessage("course instance id must be a valid integer"),
];
