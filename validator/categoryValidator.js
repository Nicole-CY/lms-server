...
const deleteCategoryByIdValidator = [
  param("id").notEmpty().withMessage("Category ID is required").isInt({ min: 1 }).withMessage("Category ID must be a valid integer"),
];

const getCategoryByIdValidator = [
  query("id").notEmpty().withMessage("Category ID is required").isInt({ min: 1 }).withMessage("Category ID must be a valid integer"),
];
...
const updateCategoryByIdValidator = [
  ...
  body("parentId")
    .optional()
    .isInt({ min: 1 }).withMessage("Parent ID must be a valid integer"),
  body("iconUrl")
    .optional()
    .isURL().withMessage("Icon URL must be a valid URL")
    .isLength({ max: 255 }).withMessage("Icon URL must not exceed 255 characters"),
